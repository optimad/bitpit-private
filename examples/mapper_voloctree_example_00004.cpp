...
void runReferenceAdaptation()
{
    ...
    /** Map data on second mesh and write */
    {
        ...
        const PiercedStorage<mapping::MappingInfo> & mapper = mapobject.getMapping();
                ...
                if (mapper[id].type == mapping::Type::TYPE_RENUMBERING){
                    ...
                    for (long idd : mapper[id].mapped){
                        ...
                        if (mapper[id].rank[i] == patch_2D->getRank()){
                            ...
                            data2[id] += datarec[mapper[id].rank[i]][idd] * volrec[mapper[id].rank[i]][idd];
                            Volume += volrec[mapper[id].rank[i]][idd];
                        }
#endif
                        i++;
                    }
    ...
    data2inv.fill(0.0);
    {
        ...
        std::map<int, std::map<long, double> > volrec;
        {
            ...
            const PiercedStorage<mapping::MappingInfo> & _mapper = mapobject.getMapping();
                ...
                if (mapper[id].type == mapping::Type::TYPE_RENUMBERING){
                    ...
                    for (long idd : mapper[id].mapped){
                        ...
                        if (mapper[id].rank[i] == patch_2D_original->getRank()){
                            ...
                            data2inv[id] += datarec[mapper[id].rank[i]][idd] * volrec[mapper[id].rank[i]][idd];
                            Volume += volrec[mapper[id].rank[i]][idd];
                        }
#endif
                        i++;
