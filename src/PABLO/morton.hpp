...
/**
* Compute the Morton number of an Octant centroid.
*
* The function comes from libmorton (see https://github.com/Forceflow/libmorton).
*
...
*/
inline uint64_t computeCentroidMorton(unsigned int x, unsigned int y, unsigned int z)
{
...
* \result The Morton number.
*/
inline uint64_t computeCentroidMorton(unsigned int x, unsigned int y)
{
...
* \result The Morton number.
*/
inline uint64_t computeNodeMorton(uint64_t x, uint64_t y, uint64_t z, int8_t maxLevel)
{
    uint64_t morton = x | (y << (maxLevel+1)) | (z << 2*(maxLevel+1));

    return morton;
}

/**
* Compute the Morton number of an Octant node.
*
* \param x is the integer x position
* \param y is the integer y position
* \param maxLevel is the Maximum allowed refinement level of octree
* \result The Morton number.
*/
inline uint64_t computeNodeMorton(uint64_t x, uint64_t y, int8_t maxLevel)
{
    uint64_t morton = x | (y << (maxLevel+1));
