 ...
 * @return identifier of new object
 */
int LevelSet::addObject( const std::vector<long> &list, int id ) {
...
/*!
 * Adds a LevelSetMask object composed of a list of interfaces and a list of interfaces to be flipped.
 * The function setMesh() must have been called prior.
 * @param[in] list list of indices of interfaces
 * @param[in] mark list of interfaces which need to be flipped
 * @param[in] id id to be assigned to object. In case default value is passed the insertion order will be used as identifier
 * @return identifier of new object
 */
int LevelSet::addObject( const std::vector<long> &list, const std::unordered_set<long> &mark, int id ) {
    ...
    m_object[id] = std::unique_ptr<LevelSetObject>( new LevelSetMask(id, list, mark, *m_kernel->getMesh()) )  ;
    addProcessingOrder(id) ;
