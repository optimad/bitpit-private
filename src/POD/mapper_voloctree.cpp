 ...
 * Get direct mapping
 */
const bitpit::PiercedStorage<mapping::Info> & MapperVolOctree::getMapping()
{
    return m_mapper;
}
 ...
 * Get inverse mapping
 */
const bitpit::PiercedStorage<mapping::Info> & MapperVolOctree::getInverseMapping()
{
    return m_invmapper;
}
                    ...
                    if (fillInv){
                        ...
                        for (long idp : (*mapperAdapted)[id].mapped){
                            ...
                            if (checkPart
                                ...
                                mapping::Info & Info = m_partitionIR.map_rank_invmapper[(*mapperAdapted)[id].rank[imapped]][idp];
                                Info.mapped.push_back(id);
                                Info.rank.push_back(info.rank);
                            }
#endif
                            imapped++;
                        }
...
void MapperVolOctree::_mappingAdaptionMappedUpdate(const std::vector<adaption::Info> & infoAdapt)
{
    ...
    PiercedStorage<mapping::Info>* mapperReference;
    ...
    if (!changedPartition){
        ...
        std::vector<adaption::Info> InfoAdapt;
#if BITPIT_ENABLE_MPI
        if (!checkPart)
            _communicateMappedAdaptionInfo(infoAdapt, InfoAdapt);
        else
#endif
            InfoAdapt = infoAdapt;
        ...
        bool fillInv, long & indRef)
{
    ...
    std::unordered_map<long, mapping::Info> _invmapper;
    ...
    if (fillInv){
        for (pair<const long, mapping::Info> & val : _invmapper){
 ...
 *
 */
void MapperVolOctree::_communicateInverseMapper(std::unordered_map<long, mapping::Info> & _invmapper, const std::vector<OctantIR> * octantsIRMapped)
{
...
void MapperVolOctree::_communicateInverseMapperBack()
{
    ...
    for (const std::pair<int, std::unordered_map<long, mapping::Info> > & val : m_partitionIR.map_rank_invmapper)
    {
        toRanks.insert(val.first);

        std::unordered_map<long, mapping::Info> _invmapper = m_partitionIR.map_rank_invmapper[val.first];
        for (const std::pair<long, mapping::Info> & sub : _invmapper)
    ...
    for (int rank : toRanks){
        std::size_t buffSize = 0;
        int mappedSize;
        std::unordered_map<long, mapping::Info> _invmapper = m_partitionIR.map_rank_invmapper[rank];
        for (long & ID : toRankID[rank]){
            mapping::Info _info = _invmapper[ID];
            mappedSize = _info.mapped.size();
            std::size_t infoBytes = std::size_t(sizeof(long) + sizeof(int) + sizeof(int) + sizeof(int) + (sizeof(long))*mappedSize);
            buffSize += infoBytes;
        }
 ...
 * \param[out] InfoAdapt Adaption info to local reference partitions
 *
 */
void MapperVolOctree::_communicateMappedAdaptionInfo(const std::vector<adaption::Info> & infoAdapt, std::vector<adaption::Info> & InfoAdapt)
{
    ...
    for (int rank : toRanks){
        ...
        int mappedSize;
        for (long & ID : toRankID[rank]){
            mapping::Info _info = m_previousmapper[ID];
            mappedSize = _info.mapped.size();
