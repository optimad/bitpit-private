...
protected:
    ...
    std::unordered_map<long, mapping::Info> m_previousmapper;   /**< Temporary mapping used during a mesh adaptation. */
    ...
    void _communicateInverseMapper(std::unordered_map<long, mapping::Info> & _invmapper, const std::vector<OctantIR> * octantsIRMapped);
    void _communicateInverseMapperBack();
    void _communicateMappedAdaptionInfo(const std::vector<adaption::Info> & infoAdapt, std::vector<adaption::Info> & InfoAdapt);
